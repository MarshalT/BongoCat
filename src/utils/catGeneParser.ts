 ...
 * @returns 解析后的基因特征
 */
export const parseGene = (gene: number): GeneParseResult => {
  // 1. 外观部分 - 位操作提取
  const appearance = {
    // 基础颜色 (当前已实现的模7算法)
    baseColor: gene % 7,
 ...
 * @returns 颜色名称
 */
export const getColorName = (colorIndex: number): string => {
  const colorNames = [
 ...
 * @returns 毛发长度名称
 */
export const getFurLengthName = (furLengthIndex: number): string => {
 ...
 * @returns 耳朵形状名称
 */
export const getEarShapeName = (earShapeIndex: number): string => {
 ...
 * @returns 眼睛颜色名称
 */
export const getEyeColorName = (eyeColorIndex: number): string => {
 ...
 * @returns 花纹名称
 */
export const getPatternName = (patternIndex: number): string => {
  const patternNames = [
 ...
 * @returns 性格名称
 */
export const getPersonalityName = (personalityIndex: number): string => {
 ...
 * @returns 稀有度名称
 */
export const getRarityName = (rarityIndex: number): string => {
  const rarityNames = [
 ...
 * @returns 成长潜力描述
 */
export const getGrowthPotentialDesc = (value: number): string => {
  let level = '';
  if (value < 70) level = '低';
 ...
 * @returns 特殊能力名称数组
 */
export const getSpecialAbilities = (abilities: number): string[] => {
  const allAbilities = [
 ...
 * @returns 猫咪基因详情
 */
export const getCatGeneDetails = (gene: number): CatGeneDetails => {
 ...
 * @returns 猫咪外观样式
 */
export const getCatAppearanceStyle = (gene: number): CatAppearance => {
  const parsedGene = parseGene(gene);
 ...
 * @returns CSS类名
 */
export const getCatColorClass = (gene: number): string => {
  const parsedGene = parseGene(gene);
  const baseColorIndex = parsedGene.appearance.baseColor;
  const colors = [
