 ...
 * 获取DFS实时价格
 * 通过API获取链上的DFS最新价格数据
 * @returns Promise<number> DFS价格
 */
export async function getDFSPrice(): Promise<number> {
  try {
    ...
    console.error('获取DFS价格失败:', error);
    // 如果获取失败，返回默认价格或上次缓存的价格
    // 这里暂时返回一个默认价格
    return 1.80; // 文件中显示的近似价格
  }
} 